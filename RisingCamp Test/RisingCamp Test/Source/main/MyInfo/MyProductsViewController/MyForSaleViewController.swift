...
class MyForSaleViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        dataManager.getForSalePrduct(self)
    }
...
extension MyForSaleViewController: UITableViewDataSource, UITableViewDelegate {
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        if indexPath.section == 0 {
            guard let topCell = tableView.dequeueReusableCell(withIdentifier: "MyForSaleCell", for: indexPath) as? MyForSaleCell else { return UITableViewCell() }
