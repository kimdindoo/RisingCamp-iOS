...
class ReservedViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        dataManager.getReservedPrduct(self)
    }
...
extension ReservedViewController: UITableViewDataSource, UITableViewDelegate {
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        if indexPath.section == 0 {
            guard let topCell = tableView.dequeueReusableCell(withIdentifier: "MyReservedCell", for: indexPath) as? MyReservedCell else { return UITableViewCell() }
